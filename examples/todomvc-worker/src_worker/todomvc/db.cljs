...
(defn todos->local-store
  ...
  [todos]
  ;; NO OP, no localsStorage in Web workers
  #_(.setItem js/localStorage ls-key (str todos)))            ;; sorted-map writen as an EDN map
...
(re-frame/reg-cofx
  ...
  (fn [cofx _]
      ...
      (assoc cofx :local-store-todos
             (into (sorted-map)
                   nil ;; NO OP, no localstorage in Web Workers!
                   #_(some->> (.getItem js/localStorage ls-key)
